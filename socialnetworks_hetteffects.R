...
data %<>%  mutate(network = paste(coar, cohort, sep="_"),
                  network_id = as.numeric(factor(network, levels = unique(network))))

covariates_data <- read_csv("covariates_data.csv") %>% 
  mutate(network = paste(coar, cohort, sep = "_"), id = as.character(id)) %>% 
  rename(idego = id) %>% dplyr::select(-c(coar, cohort))

data %<>% left_join(covariates_data, by = c("network", "idego"))
...
names <- c("Shyness",  "Friendlyness")    # vector of labels for outcome variables
Y     <- c("shy_end", "friendly_end")     # vector of outcome variables
D     <- rep("Hsocial_exp", length(Y))    # vector of treatment variables

# specify cluster, fixed effect and partition
cluster      <- "network"       # if no cluster       use    cluster      <- "0"
fixed_effect <- "groupfe"         # if no fixed_effect  use    fixed_effect <- "0"
partition    <- "0"       # if no partition     use    partition    <- "0"

# create a vector of control variables
controls     <- c("centrality_base", "male", "poor_score", "admission_test")

affected       <- c("poor_score", "shy_base", "friendly_base")      # characteristics for most/least affected analysis
names_affected <- c("poor_score", "shy_base", "friendly_base")      # characteristics for most/least affected analysis
...
if(fixed_effect!="0" & cluster!="0"){
  data <- data[,c(Y, D, controls, cluster, fixed_effect, affected)]
}
...
r <- foreach(t = 1:sim, .combine='cbind', .inorder=FALSE, .packages=packages) %do% { 
...
#  dimension of these three vectors should match. If dimension is greater than 1 the program runs heterogeneity estimation separately for each outcome variable
names <- c("Extroversion",  "Agreeableness")    # vector of labels for outcome variables
Y     <- c("shy_end", "friendly_end")     # vector of outcome variables
D     <- rep("Hsocial_exp", length(Y))  # vector of treatment variables
...
# create a vector of control variables
controls     <- c("Hcognitive_exp", "centrality_base", "male", "poor_score")
X <- ""
...
if(fixed_effect=="0" & cluster=="0"){
  data <- data[,c(Y, D,controls, partition)]
}

if(fixed_effect=="0" & cluster!="0"){
  data <- data[,c(Y, D,controls, cluster, partition)]
}

if(fixed_effect!="0" & cluster=="0"){
  data <- data[,c(Y, D,controls, fixed_effect, partition)]
}

if(fixed_effect!="0" & cluster!="0"){
  data <- data[,c(Y, D, controls, cluster, fixed_effect, partition)]
}
...
for(i in 1:sim){
  ...
  print(i)
  if(partition!="0"){
    ind <- createDataPartition(data[,partition], p = .5, list = FALSE) #random partition
    
    datause <- as.data.frame(data[ ind,]) #training data for simulation
    dataout <- as.data.frame(data[-ind,]) #testing data for simulation
  }
  
  treateduse <- which(datause$Hsocial_exp==1) #treatment units
  dataused <- data.frame(datause[treateduse, c(controls, y)]) #treated in training
  datausend <- data.frame(datause[-treateduse, c(controls, y)]) #control in training
  
  form <- as.formula(paste(y,"~",x,sep=""));
  RF_treated <- randomForest(form, data=dataused, ntree=1000, importance=TRUE, mtry=3) #RF treated
  RF_control <- randomForest(form, data=datausend, ntree=1000, importance=TRUE, mtry=3) #RF control
  
  pred_treatment <- predict(RF_treated, newdata = dataout) #prediction treated
  pred_control <- predict(RF_control, newdata = dataout) #prediction control
  
  dataf <- data.frame(dataout, pred_treatment, pred_control) #predictions
  namefile <- paste('ATERF', i, '.dta', sep="") 
  write.dta(dataf, namefile) #export to stata
}
